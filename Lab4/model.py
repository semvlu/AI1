...
def check_all_models():
    ...
    model = []
    for p in itertools.product([True,False],repeat=len(identifiers)):
        model.append(dict(zip(identifiers,p)))
    
    for i in model:
        kb_eval = evaluate_expression_set(kb, i)
        infer_eval = evaluate_expression_set(infer, i)
        if kb_eval and not infer_eval:
            print("KB does not entail INFER, counter example: ", i)
            return False
    return True
