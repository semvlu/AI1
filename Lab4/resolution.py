...
def size(clause):
    return len(clause.positive) + len(clause.negative)

def recursive_print_proof(idx, clause_set, inferred):
    printer = []
    stack = []
    OG = []
    ...
    for i in OG: # push the clause w/ only a literal to the stack
        if size(i) == 1:
            stack.append(i)
            break
    ...
    for i in inferred: # find complementary in the 1e step
        if size(i) == 1:
            if positive == True:
                if len(i.negative) == 1:
                    if i.negative[0] == tmp.positive[0]:
                        stack.append(i)
            else:
                if len(i.positive) == 1:
    ...
    while entry == True or 
        ...
        for i in clause_set: # push the clauses that contain the literals in the popped clause
            if positive == True:
                for j in i.positive:
...
def print_proof(clause_set, inferred):
    empty_clause = Clause("")
    idx = find_index_of_clause(empty_clause, clause_set)
    recursive_print_proof(idx, clause_set, inferred)
