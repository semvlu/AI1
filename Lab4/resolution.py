...
def init():
    ...
    stdin = stdin.replace('KB=', "")

    stdin = stdin[1:len(stdin)-1]
    
    try:
        for i in range(len(stdin)-1):
            if stdin[i] == "," and stdin[i-1] == "]" and stdin[i+1] == "[" and i > 1 and i < (len(stdin)-1):
...
def recursive_print_proof(idx, clause_set, og):
    
    printer = []
    for i in og:
        for j in clause_set:
            if can_resolve(i, j) and resolve_clauses(i,j).equals(clause_set[idx]) and clause_set[idx].proven == False:
                
                printer.append(clause_set[idx].print_store() + " is inferred from "+ i.print_store() + " and "+ j.print_store() +".")
                clause_set[idx].proven = True
                if is_element_of_clause_set(j, og):
                    break
                recursive_print_proof(find_index_of_clause(j, clause_set), clause_set, og)
...
def print_proof(clause_set, og):
    empty_clause = Clause("")
    idx = find_index_of_clause(empty_clause, clause_set)
    recursive_print_proof(idx, clause_set, og)
